...
defined('PAR_EXEC') OR die('Payzito Restricted Access');

class jdate extends JDFDateParent
{
    const IRAN_TIME_ZONE = 'Asia/Tehran';
    const EUROPE_TIME_ZONE = 'Europe/Berlin';

    function getJalaiDate($format=null,$timestamp='')
    {
        if($format === null)
        {
            $format = 'Y/m/d H:i:s';
        ...
        return $this->jdate($format,$timestamp,'',self::IRAN_TIME_ZONE,'en');
    }

    function gregorianToJalali($date,$format='Y/m/d H:i:s')
    {
        return $this->jdate($format,$this->strToTime($date),'',self::IRAN_TIME_ZONE,'en');
    }

    function jalaliToGregorianDateTime($date)
    {
        $date = explode(' ',$date);
        if(count($date) != 2)
        {
        ...
        if(count($time1) != 3 || count($time2) != 3)
        {
            return '';
        }

        $gdate = $this->jalali_to_gregorian($time1[0],$time1[1],$time1[2],'-');
        date_default_timezone_set(self::IRAN_TIME_ZONE);
        $time = strtotime($gdate.' '.implode(':',$time2));
        date_default_timezone_set(self::EUROPE_TIME_ZONE);
    ...
    function getGregorianDate($format='Y/m/d H:i:s',$ts='',$timeZone='')
    {
        if(!empty($timeZone))
        {
    ...
    {
        return substr($date,0,11).' '.'00:00:00';
    }
    ...
    function getIranTimeStamp()
    {
        $date = new DateTime("now",new DateTimeZone('Asia/Tehran'));
        return $date->getTimestamp() + $date->getOffset();
    }

    function words($array,$mod='')
    {
        return parent::jdate_words($array,$mod);
    }

    function convertNumber($str,$mod='en',$mf='٫')
    {
        return parent::tr_num($str,$mod,$mf);
    }

    function getDateInMonthAgo($date,$month=1)
    {
        $date = explode('/',$date);
        if(count($date) == 3)
        {
        ...
        return $date[0].'/'.($date[1]<10 ? '0'.(int)$date[1] : $date[1]);
    }

    function modifyDate($date)
    {
        if(strpos($date,'سهم') !== false)
        {
            $date = str_replace('سهم','سوم',$date);
        }
        if(strpos($date,'سیم') !== false)
        {
            $date = str_replace('سیم','سی ام',$date);
        }

        return $date;
    }

    function dateRanges($startDate,$endDate,$repeat='daily')
    {
            ...
            if($condition)
            {
                switch ($repeat)
                {
                    case 'yearly':
                        ...
                        break;
                    case 'monthly':
                        $array[] = $start[0].'/'.($start[1]<10 ? '0'.(int)$start[1] : $start[1]);
                        break;
                    case 'daily':
                        $array[] = $start[0].'/'.($start[1]<10 ? '0'.(int)$start[1] : $start[1]).'/'.($start[2]<10 ? '0'.(int)$start[2] : $start[2]);
                        break;
                }
            }
    ...
    function checkJalaliDate($date,$sep='-',$defaultDate='')
    {
        if(empty($defaultDate))
        {
            $defaultDate = $this->getJalaiDate('Y'.$sep.'m'.$sep.'d');
        }
        $defaultDate = explode($sep,$defaultDate);

        $date = explode($sep,$date);
        if(!isset($date[0]) || empty($date[0]) || $date[0]<1380 || $date[0]>1420)
        {
            $date[0] = $defaultDate[0];
        }
        if(!isset($date[1]) || empty($date[1]) || $date[1]<0 || $date[1]>12)
        {
            $date[1] = $defaultDate[1];
        }
        if(!isset($date[2]) || empty($date[2]) || $date[2]<0 || $date[2]>31)
        {
            $date[2] = $defaultDate[2];
        }

        return implode($sep,$date);
    }

    function fillByZeroValues($array)
    {
        if(empty($array))
        {
            return [];
    ...
    function jalaliStrToTime($time)
    {
        $gDate = $this->jalaliToGregorianDateTime($time);
        return strtotime($gDate);
    }

    function parseSecondTime($time)
    {
    ...
    function timeDifference($time1,$time2=null)
    {
        $time2 = !is_null($time2) ? $time2 : time();

        $diff = $time1 - $time2;
        $diff = $diff > 0 ? $diff : 0;

        return array_merge(['diff' => $diff],$this->parseSecondTime($diff));
    }
}
...
class JDFDateParent
{
    protected function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa') {
    ...
    protected function jmktime($h = '', $m = '', $s = '', $jm = '', $jd = '', $jy = '', $none = '', $timezone = 'Asia/Tehran') {
    ...
    protected function jgetdate($timestamp = '', $none = '', $timezone = 'Asia/Tehran', $tn = 'en') {
    ...
    protected function gregorian_to_jalali($gy, $gm, $gd, $mod = '') {
    ...
    protected function jalali_to_gregorian($jy, $jm, $jd, $mod = '') {
